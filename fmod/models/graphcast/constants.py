...
from fmod.base.util.config import cfg2meta
...
class Constants(BaseModel):
    ...
    icospheres_path: str = os.path.join( Path(__file__).parent.resolve(), "icospheres.json" )

def get_constants() -> Constants:
    c = cfg2meta( "model", Constants() )
    if c.cugraphops_encoder or c.cugraphops_processor or c.cugraphops_decoder:
        try:
            import pylibcugraphops
        except:
            raise ImportError("pylibcugraphops is not installed.")
    return c
