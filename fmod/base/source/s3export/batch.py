...
class S3ExportDataLoader(SRDataLoader):
	...
	def __init__(self, task_config: DictConfig, vres: str, **kwargs):
		...
		self.tile_size: int = self.task.tile_size
	...
	def cut_xy_coords(self, origin: Tuple[int,int] )-> Dict[str,xa.DataArray]:
		tcoords: Dict[str,np.ndarray] = { c:  self.cut_coord( self.ijc[c], origin[idx] ) for idx, c in enumerate(['i','j']) }
	...
	def load_dataset(self, d: datetime, vres: str):
		raise NotImplementedError("SRDataLoader:load_norm_data")
