...
class SWOTRawDataLoader(SRRawDataLoader):
	...
	def get_tiles(self, var_data: List[np.ndarray]) -> xa.DataArray:
		...
		result = ctiles.reshape( ntiles//ishape['c'], ishape['c'], tsize['y'], tsize['x'] )
		print(f" ---- tiles{tiles.shape}, tile_idxs{tile_idxs.shape}, ctiles{ctiles.shape} -> result{result.shape}")
