...
class SWOTRawDataLoader(SRRawDataLoader):
	...
	def get_tiles(self, var_data: List[np.ndarray]) -> xa.DataArray:
		...
		lgm().log( f"get_tiles{tsize}, grid_shape{grid_shape}, roi{roi}, ishape{ishape}, region_data{region_data.shape}",display=True)
		tile_data = region_data.reshape( ishape['c'], grid_shape['y'], tsize['y'], grid_shape['x'], tsize['x'] )
		tiles = np.swapaxes(tile_data, 2, 3).reshape(  grid_shape['y'] * grid_shape['x'], ishape['c'], tsize['y'], tsize['x'])
		msk = np.isfinite( tiles[:,0,:,:].mean(axis=-1).mean(axis=-1) )
		ntiles = np.count_nonzero(msk)
		print( f"get_tiles: tiles{tiles.shape}, msk{msk.shape}, mnz={ntiles} ")
		result = np.compress( msk, tiles,0)
		tile_idxs = np.compress( msk, np.arange(tiles.shape[0]), 0)
		print(f" ---- tiles{tiles.shape}, tile_idxs{tile_idxs.shape}, result{result.shape}")
